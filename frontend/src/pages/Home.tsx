import { useEffect, useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { api } from '../api/client';
import { ApiResponse, Category, EventDto } from '../api/types';
import EventCard from '../components/EventCard';
import TicketShowcase from '../components/TicketShowcase';
import { useT } from '../i18n';

export default function Home() {
  const [featured, setFeatured] = useState<EventDto[]>([]);
  const [upcoming, setUpcoming] = useState<EventDto[]>([]);
  const [cats, setCats] = useState<Category[]>([]);
  const [q, setQ] = useState('');
  const nav = useNavigate();
  const t = useT();

  useEffect(() => {
    api.get<ApiResponse<EventDto[]>>('/events/featured').then(r => setFeatured(r.data.data));
    api.get<ApiResponse<EventDto[]>>('/events/upcoming').then(r => setUpcoming(r.data.data));
    api.get<ApiResponse<Category[]>>('/categories').then(r => setCats(r.data.data));
  }, []);

  const search = (e: React.FormEvent) => { e.preventDefault(); nav(`/events?q=${encodeURIComponent(q)}`); };

  return (
    <div>
      <section className="relative overflow-hidden bg-gradient-to-b from-brand-50/60 to-white">
        <div className="container-x relative pt-12 pb-10 md:pt-16 md:pb-12">
          <div className="text-center max-w-3xl mx-auto">
            <span className="chip-brand mb-5 inline-block">{t('hero.badge')}</span>
            <h1 className="text-4xl md:text-6xl font-extrabold leading-[1.05] text-slate-900">
              {t('hero.title1')}<br/><span className="text-brand-600">{t('hero.title2')}</span>
            </h1>
            <p className="mt-5 text-lg text-slate-600 mx-auto max-w-2xl">{t('hero.desc')}</p>
            <form onSubmit={search} className="mt-8 flex gap-2 max-w-xl mx-auto">
              <input value={q} onChange={e=>setQ(e.target.value)} className="input flex-1" placeholder={t('hero.search')} />
              <button className="btn-primary whitespace-nowrap">{t('hero.searchBtn')}</button>
            </form>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-slate-500">
              <span>{t('hero.f1')}</span>
              <span>{t('hero.f2')}</span>
              <span>{t('hero.f3')}</span>
            </div>
          </div>
          <div className="relative mt-14 md:mt-16">
            <TicketShowcase events={featured} />
          </div>
        </div>
      </section>

      <section className="container-x pt-10 pb-6">
        <div className="flex items-end justify-between mb-5">
          <h2 className="text-2xl font-bold text-slate-900">{t('cat.title')}</h2>
          <Link to="/events" className="text-brand-700 text-sm hover:underline">{t('cat.all')}</Link>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          {cats.map(c => (
            <Link key={c.id} to={`/events?categoryId=${c.id}`}
              className="card card-hover p-5 text-center group">
              <div className="text-2xl group-hover:scale-110 transition">🎭</div>
              <div className="mt-2 text-sm font-medium text-slate-700 group-hover:text-brand-600">{c.name}</div>
            </Link>
          ))}
        </div>
      </section>

      <section className="container-x pt-6 pb-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-2xl font-bold text-slate-900">{t('trending.title')}</h2>
            <p className="text-sm muted mt-1">{t('trending.sub')}</p>
          </div>
          <Link to="/events" className="text-brand-700 text-sm hover:underline">{t('trending.all')}</Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {featured.map(e => <EventCard key={e.id} e={e} />)}
        </div>
      </section>

      <section className="container-x pt-6 pb-10">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-2xl font-bold text-slate-900">{t('upcoming.title')}</h2>
            <p className="text-sm muted mt-1">{t('upcoming.sub')}</p>
          </div>
          <Link to="/events" className="text-brand-700 text-sm hover:underline">{t('trending.all')}</Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {upcoming.map(e => <EventCard key={e.id} e={e} />)}
        </div>
      </section>

      <section className="container-x py-10">
        <div className="rounded-3xl p-10 md:p-14 bg-brand-500 text-slate-900 relative overflow-hidden">
          <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_top_right,#fff_0%,transparent_50%)]" />
          <div className="relative grid md:grid-cols-2 gap-6 items-center">
            <div>
              <h3 className="text-3xl md:text-4xl font-extrabold">{t('cta.title')}</h3>
              <p className="mt-3 opacity-90">{t('cta.desc')}</p>
            </div>
            <div className="flex gap-2">
              <input className="input bg-white/90 border-white/50 text-slate-900 placeholder-slate-500 focus:bg-white" placeholder="Your email" />
              <button className="btn-dark whitespace-nowrap">{t('cta.subscribe')}</button>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
